...
The good news is thatMonELOG supports a wide variety of readily available 
...
The CTs that are sold at the `MonELOG stuff store <http://stuff.MonELOG.com>`_  
...
The 3.5mm connectors plug into any of the 14 input channels on theMonELOG.
...
Not to worry, in the event some end up backward,MonELOG will still work, 
...
At this point, you should have theMonELOG up and running with the voltage 
...
**Allow negative power value**
    ...
    grid-tied solar (net-metering).  Checking this box tellsMonELOG 
...
**Reverse**
    ...
    current flow.  
   MonELOG will sense this and correct automatically in single-phase 
    ...
    is a necessity in three-phase installations because theMonELOG 
...
Each time you press save, the new configuration is sent toMonELOG 
...
When you have configured all of the CTs connected to theMonELOG, 
basic configuration is complete.
Click the |Status| button to see theMonELOG in action.
...
MonELOG. If your particular CT is not one of the dropdown models, 
...
Current type CTs are the most common type of CT used withMonELOG and
all of the CTs available in theMonELOG *Stuff* Store are of this type. 
...
voltage rather than current.  They are connected to a modifiedMonELOG 
input that has had the internal burden resistor removed and specified as zero 
in the device configuration burden menu. MonELOG will ask for a **Cal** 
