...
MonELOG has the capability to measure power in polyphase systems. 
...
until needed. There has been a lot of interest in using MonELOG
...
MonELOG can also be used with "three wire" or "delta" systems using 
different methods.
...
MonELOG can do this by two different methods: 
...
This chapter explains how to configure MonELOG to 
...
Connect the CTs to the MonELOG and use the `inputs` setup to
set the CT model. Check the *allow negative values* box for each.

Configure the VT
^^^^^^^^^^^^^^^^

Set up your MonELOG with the voltage reference VT on whatever 
phase of the three-phase is convenient. We will call that phase
A. This is important:

    *However you identify your physical phases, MonELOG always 
...
While watching the power draw of the three mains on the MonELOG status display,
...
Version 5 of MonELOG has plugs to connect the additional VTs.
