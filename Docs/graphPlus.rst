...
any of the data stored in the MonELOG. It features relative and absolute time period selection 
and handles all units of measure supported by MonELOG. You can produce line and/or bar charts 
...
On the right Watts are selected and this MonELOG lists 15 sources that are configured 
...
MonELOG records each measurement at 5 second intervals. That's 17,280
measurements per day. The graph size cannot possibly represent that, and it would take
a long time to query that amount of data.  Instead, MonELOG can deliver either
...
Each graph is saved in the MonELOG, so it doesn't matter if you use a different 
...
Graph+ can be loaded directly from the MonELOG's web server using the URL

http://MonELOG.local/graph2.htm [?graph=savedgraph [&embed] ]
