...
MonELOG fully supports the influxDB HTTP API for uploading data 
to influxDB at a specific interval of 5 seconds to one hour. 
Like other similar MonELOG services, continuity of updates is 
...
to version 2, depending on the implementation (cloud or OSS), but MonELOG
...
To configure the influxDB upload service in MonELOG, 
...
**post interval**
    ...
    MonELOG accepts any value from 5 seconds to 3600 seconds (1 hour), 
...
**bulk send**
    ...
    MonELOG will aggregate the posting data for the specified number 
    ...
    MonELOG keeps all of the data in the SD data log, 
    so there is no risk of losing data by using this feature. 
    Should there be any failure to deliver, 
    MonELOG will pick up with the last successful posting when the problem is resolved.
...
**upload history from**
    Specify the starting date that MonELOG will use to upload history 
...
**measurement**
    Name that you assign to the measurements that MonELOG will be posting. 
...
**database**
    ...
    have created to be the repository for the MonELOG data.
...
**username/password** 
    Optional security credentials. If specified, MonELOG will 
...
**Authorization token**
    Authorization token produced by influx. (Data->Tokens->Generate Token-> Read/Write Token for MonELOG bucket)
...
**tag-set** 
    ...
    The first tag-set is a special case for MonELOG, and if specified, 
...
**measurements** 
    ...
    create scripts to generate data using the various MonELOG inputs. 
...
There are three variables defined: 
    ...
    *   **$device** - The name assigned to this MonELOG device 
