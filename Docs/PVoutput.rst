...
real-time alerts based on simple rules. MonELOG supports 
...
So lets look at how to set this up with MonELOG.

First, you should review the local time offset specified in your MonELOG. 
MonELOG synchronizes with PVoutput using local time, 
including daylight time where applicable. 
In the Setup/Device display of MonELOG, 
...
Your account must match the local time setting of your MonELOG. 
Click Save at the bottom and your account is setup. 
Note that this page is where you will find your API Key, 
which is one of the few things you will need to specify in the MonELOG setup.
...
check that the timezone matches what you have set in MonELOG. 
Click Save. Note the System ID. MonELOG will need to know that.

Configure MonELOG
------------------

It's time to configure MonELOG to upload to the new account. 
From within the MonELOG configuration app, 
...
**generation** 
    ...
    Select the MonELOG input that measures your inverter output.
